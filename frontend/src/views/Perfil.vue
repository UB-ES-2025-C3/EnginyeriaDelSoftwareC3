        ...
        <div class="grid grid-cols-3 items-center gap-4">
          ...
          <div class="flex items-center justify-end gap-4">
            <router-link v-if="isLoggedIn" to="/perfil"
              class="w-10 h-10 rounded-full bg-gray-800 hover:bg-gray-700 flex items-center justify-center transition-colors overflow-hidden border-2 border-purple-500">
      ...
      <div class="max-w-4xl mx-auto">
        ...
        <form @submit.prevent="handleSave" class="space-y-8 sm:space-y-10">
          ...
          <div class="space-y-2">
            ...
            <div
              class="h-40 sm:h-48 border-2 border-dashed border-gray-600 rounded-xl flex items-center justify-center bg-gray-900/30 overflow-hidden">
              <img v-if="bannerPreview" :src="bannerPreview" alt="Previsualització portada"
            ...
            <div class="flex gap-3">
              <button type="button"
                class="px-4 py-2 text-sm border border-gray-600 rounded-lg hover:bg-gray-800 transition-all font-semibold uppercase tracking-wide"
                @click="bannerInput?.click()">Canviar portada</button>
              <button v-if="bannerPreview" type="button"
                class="px-4 py-2 text-sm border border-gray-600 rounded-lg hover:bg-gray-800 transition-all font-semibold uppercase tracking-wide"
                @click="clearBanner">Treure</button>
            </div>
          ...
          <div class="space-y-2">
            ...
            <div class="flex items-center gap-4">
              <div
                class="w-20 h-20 sm:w-24 sm:h-24 rounded-full border-2 border-gray-600 overflow-hidden flex items-center justify-center bg-gray-800">
                <img v-if="avatarPreview" :src="avatarPreview" alt="Previsualització avatar"
                  class="w-full h-full object-cover" />
              ...
              <div class="flex gap-3">
                ...
                <button type="button"
                  class="px-4 py-2 text-sm border border-gray-600 rounded-lg hover:bg-gray-800 transition-all font-semibold uppercase tracking-wide"
                  @click="avatarInput?.click()">Canviar avatar</button>
                <button v-if="avatarPreview" type="button"
                  class="px-4 py-2 text-sm border border-gray-600 rounded-lg hover:bg-gray-800 transition-all font-semibold uppercase tracking-wide"
                  @click="clearAvatar">Treure</button>
              </div>
            </div>
            <small class="text-xs text-gray-500">JPG/PNG, màx 2 MB</small>
          </div>
          ...
          <div class="space-y-6">
            ...
            <div class="space-y-2">
              <label for="name" class="block text-sm font-semibold text-gray-300 uppercase tracking-wide">Nom
                d'usuari</label>
              <input id="name" v-model.trim="name" type="text" placeholder="El teu nom"
            ...
            <div class="space-y-2">
              <label for="bio"
                class="block text-sm font-semibold text-gray-300 uppercase tracking-wide">Biografia</label>
              <textarea id="bio" v-model="bio" placeholder="Explica'ns una mica sobre tu..." maxlength="250"
            ...
            <div class="space-y-2">
              <label for="email" class="block text-sm font-semibold text-gray-300 uppercase tracking-wide">Correu
                electrònic</label>
          ...
          <div class="space-y-6">
            ...
            <div class="grid sm:grid-cols-2 gap-4 sm:gap-6">
              <div class="space-y-2">
                <label for="steam" class="block text-sm font-semibold text-gray-300 uppercase tracking-wide">Steam
                  (URL)</label>
                <input id="steam" v-model="links.steam" type="url" placeholder="https://steamcommunity.com/..."
                  class="w-full px-4 py-3 bg-transparent border-2 border-gray-600 rounded-xl text-white placeholder-gray-500 focus:outline-none focus:border-gray-400 transition-all" />
              </div>
              <div class="space-y-2">
                <label for="twitch" class="block text-sm font-semibold text-gray-300 uppercase tracking-wide">Twitch
                  (URL)</label>
                <input id="twitch" v-model="links.twitch" type="url" placeholder="https://twitch.tv/..."
                  class="w-full px-4 py-3 bg-transparent border-2 border-gray-600 rounded-xl text-white placeholder-gray-500 focus:outline-none focus:border-gray-400 transition-all" />
              </div>
              <div class="space-y-2">
                <label for="psn" class="block text-sm font-semibold text-gray-300 uppercase tracking-wide">PSN
                  ID</label>
                <input id="psn" v-model="links.psn" type="text" placeholder="El teu ID de PSN"
                  class="w-full px-4 py-3 bg-transparent border-2 border-gray-600 rounded-xl text-white placeholder-gray-500 focus:outline-none focus:border-gray-400 transition-all" />
              </div>
              <div class="space-y-2">
                <label for="xbox" class="block text-sm font-semibold text-gray-300 uppercase tracking-wide">Xbox
                  Gamertag</label>
          ...
          <!-- Actions -->
          <div
            class="flex flex-col-reverse sm:flex-row items-stretch sm:items-center justify-end gap-4 pt-6 border-t border-gray-700">
            <p v-if="error"
              class="text-red-400 text-sm bg-red-900/20 border border-red-800 rounded-lg px-4 py-3 w-full sm:w-auto sm:mr-auto">
              {{ error }}</p>
            <p v-if="success"
