...
onMounted(async () => { if (!auth.state.token) return; loading.value = true; try { const userData = await api.getProfile(auth.state.token); originalData.value = JSON.parse(JSON.stringify(userData)); resetForm(); } catch (e) { error.value = 'No s\'ha pogut carregar el teu perfil'; } finally { loading.value = false; } });
